...
{
    public class SanctionAddressesBKTree
    {
        ...
        public class AddressSearchResult
        {
            ...
            public AddressSearchResult(string entryId, string matched, double similarity, int editDistance)
            {
                EntryId = entryId;
                Matched = matched;
        ...
        public SanctionAddressesBKTree(double threshold = 0.90, bool caseSensitive = false)
        {
        ...
        public void Load(IEnumerable<SanctionEntry> entries)
        {
            ...
            {
                foreach (var addr in entry.Addresses.Where(a => !string.IsNullOrWhiteSpace(a)))
                    Insert(Normalise(addr), entry.ID);
            }
        }

        public List<AddressSearchResult> Search(string query, double? threshold = null)
        {
            if (_root == null) return new List<AddressSearchResult>();
            if (string.IsNullOrWhiteSpace(query)) return new List<AddressSearchResult>();
        ...
        private void RecursiveSearch(BKNode node, string query, List<AddressSearchResult> results, double threshold)
        {
            ...
            if (dist <= maxAllowed && maxLen > 0)
            {
                ...
                results.Add(new AddressSearchResult(node.EntryId, node.Key, similarity, dist));
            }
