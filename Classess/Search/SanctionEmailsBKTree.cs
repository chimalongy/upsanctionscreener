...
{
    public class SanctionEmailsBKTree
    {
        ...
        public class EmailSearchResult
        {
            ...
            public EmailSearchResult(string entryId, string matched, double similarity, int editDistance)
            {
                EntryId = entryId;
                Matched = matched;
        ...
        public SanctionEmailsBKTree(double threshold = 0.90, bool caseSensitive = false)
        {
        ...
        public void Load(IEnumerable<SanctionEntry> entries)
        {
            ...
            {
                foreach (var email in entry.EmailAddresses.Where(e => !string.IsNullOrWhiteSpace(e)))
                    Insert(Normalise(email), entry.ID);
            }
        }

        public List<EmailSearchResult> Search(string query, double? threshold = null)
        {
            if (_root == null) return new List<EmailSearchResult>();
            if (string.IsNullOrWhiteSpace(query)) return new List<EmailSearchResult>();
        ...
        private void RecursiveSearch(BKNode node, string query, List<EmailSearchResult> results, double threshold)
        {
            ...
            if (dist <= maxAllowed && maxLen > 0)
            {
                ...
                results.Add(new EmailSearchResult(node.EntryId, node.Key, similarity, dist));
            }
